...
Set this provider at the root of the project if you want to show/hide a splash screen at will, similarly to `AlertsProvider`.
...
```
show(options)   // to trigger the visibility of the splash screen
hide()          // to hide the splash screen (resets options)
update(options) // to update the options without changing the visibility
```
...
export const Demo = () => {
  ...
  useEffect(() => {
    const handlePressKey = (e) => {
      if (e.code === 'KeyS') {
        visible ? hide() : show({ text: 'Loading...' });
        toggle(! visible);
      }
    };
