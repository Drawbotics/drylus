...
import { UploadBox, UploadHelper, Button, Category, Icon, Margin, Size, AttachmentBox } from '@drawbotics/react-drylus';
...
The component is most often coupled with the [`AttachmentBox`](/component-kit/components/attachment-box) and `AttachmentList` component to display the upload progress.
    ...
    {({ state, setState }) => (
      <div style={{ width: 400 }}>
        ...
        {Object.keys(state).map((filename) => (
          <Margin key={filename} size={{ bottom: Size.EXTRA_SMALL }}>
            <AttachmentBox
