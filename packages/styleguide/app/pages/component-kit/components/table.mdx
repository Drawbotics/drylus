...
<Playground>
  <div style={{ background: '#eaeff4', padding: '20px', margin: '-24px' }}>
    <Table isLoading header={[ 'age', 'height', 'something', 'else' ]} />
  </div>
</Playground>


### Table without content
You can set `emptyContent` to replace the table content and show something else. This is useful when there is no data that can be displayed in the table. By default you should use the [`EmptyState`](/components/empty-state) component, but you can pass anything you want.
