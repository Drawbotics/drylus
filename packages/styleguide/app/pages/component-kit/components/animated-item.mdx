...
Used in conjunction with `AnimationGroup` (see below) this component allows you to add animations to the UI easily. Using `framer-motion` under the hood most of the animation should work automatically. There are a couple of props available to customize the animation to better suit the interface.
...
Internally the `AnimationGroup` uses `AnimatePresence` from framer-motion when the `animateExit` prop is given, that's why the component should be placed outside of the content that is currently being toggled (enter/exit).
